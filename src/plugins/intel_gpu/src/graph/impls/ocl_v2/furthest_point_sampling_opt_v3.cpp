...
//
#include "furthest_point_sampling_opt_v3.hpp"
...
class FurthestPointSamplingGeneratorOptV3 : public KernelGenerator {
public:
    FurthestPointSamplingGeneratorOptV3() : KernelGenerator("furthest_point_sampling_opt_v3") {}
...
protected:
    ...
    [[nodiscard]] DispatchDataFunc get_dispatch_data_func() const override {
        return DispatchDataFunc{[](const RuntimeParams& params, KernelData& kd, ImplRuntimeParams* /*rt_params*/) {
            ...
            // Dynamic work-group size based on N for better performance
...
class FurthestPointSamplingOptV3Impl : public PrimitiveImplOCL {
public:
    DECLARE_OBJECT_TYPE_SERIALIZATION(ov::intel_gpu::ocl::FurthestPointSamplingOptV3Impl)
    Stage::Ptr grouping_stage = make_stage<FurthestPointSamplingGeneratorOptV3>();

    FurthestPointSamplingOptV3Impl() : PrimitiveImplOCL(FurthestPointSamplingOptV3::get_type_info_static()) {}
    FurthestPointSamplingOptV3Impl(const program_node& /*node*/, const RuntimeParams& params) : FurthestPointSamplingOptV3Impl() {
        add_stage(grouping_stage, params);
    }

    [[nodiscard]] std::unique_ptr<primitive_impl> clone() const override {
        return make_deep_copy<FurthestPointSamplingOptV3Impl>(this);
    }
...
std::unique_ptr<primitive_impl> FurthestPointSamplingOptV3::create_impl(const program_node& node, const RuntimeParams& params) const {
    assert(node.get_dependencies().size() == 2);  // features, idx
    assert(node.get_outputs_count() == 1);        // output
    return std::make_unique<FurthestPointSamplingOptV3Impl>(node, params);
}

}  // namespace ov::intel_gpu::ocl

BIND_BINARY_BUFFER_WITH_TYPE(ov::intel_gpu::ocl::FurthestPointSamplingOptV3Impl)
