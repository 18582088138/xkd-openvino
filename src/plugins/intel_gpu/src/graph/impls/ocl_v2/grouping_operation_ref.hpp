...
#include "grouping_operation_base.hpp"
...
struct GroupingOperationRef : public GroupingOperationBase {
    OV_GPU_PRIMITIVE_IMPL("ocl::grouping_operation::ref")
    explicit GroupingOperationRef(shape_types shape_type, ValidateFunc vf = nullptr) : GroupingOperationBase(shape_type, std::move(vf)) {}
    ...
    [[nodiscard]] bool validate_impl(const program_node& node) const override {
        ...
        const auto& idx_layout = node.get_input_layout(1);       // idx (B, npoint, nsample)
        const auto& out_layout = node.get_output_layout(0);      // output (B, C, npoint, nsample)
